    ...
    private val imageUri: Uri?
        get() = when {
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q -> {
                ...
                com.infinity_it_solution_assement.WebViewAppApplication.getContext().contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values)
            }
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.N -> {
                try {
                    val storageDir = com.infinity_it_solution_assement.WebViewAppApplication.getContext().getExternalFilesDir(Environment.DIRECTORY_DCIM)
                    val photoFile = File.createTempFile(System.currentTimeMillis().toString(), ".jpg", storageDir)
                    FileProvider.getUriForFile(
                            com.infinity_it_solution_assement.WebViewAppApplication.getContext(),
                            com.infinity_it_solution_assement.WebViewAppApplication.getContext().packageName + ".fileprovider",
                            photoFile)
                } catch (e: IOException) {
                    e.printStackTrace()
                    null
